...
- Throughout the repository, the name `Isaac_drone` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of Isaac_drone (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/Isaac_drone
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/Isaac_drone/Isaac_drone/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/Isaac_drone"
    ]
}
```
